...
const sampleProducts = [
  {
    ...
    sku: 'GEL-45L-001',
    description:
      'Geladeira portátil de 45 litros, funciona em 12V e 24V. Ideal para viagens longas. Baixo consumo de energia e congelamento rápido até -20°C.',
    image:
  ...
  },
  {
    ...
    sku: 'AC-24V-TETO',
    description:
      'Ar condicionado elétrico de teto para caminhões. Funciona com motor desligado. Instalação universal.',
    image:
  ...
  },
  {
    ...
    sku: 'FRE-KIT-001',
    description:
      'Kit completo de freio a ar para carretas. Inclui válvulas, conexões e tubulação. Alta durabilidade e segurança.',
    image:
      'https://images.unsplash.com/photo-1486262715619-01b80258e0a5?auto=format&fit=crop&q=80&w=800',
    specifications: [
  ...
  },
  {
    ...
    sku: 'SUS-BOL-D01',
    description:
      'Bolsa de ar reforçada para suspensão dianteira de caminhões pesados. Maior conforto e estabilidade.',
    image:
      'https://images.unsplash.com/photo-1517055729445-41e9200cff0c?auto=format&fit=crop&q=80&w=800',
    specifications: [
  ...
  },
  {
    ...
    sku: 'LED-MIL-60W',
    description:
      'Par de faróis de milha em LED de alta potência. Corpo em alumínio e lente de policarbonato. Resistente à água IP67.',
    image:
  ...
  },
  {
    ...
    sku: 'KIT-FER-PRO',
    description:
      'Maleta de ferramentas completa para manutenção básica na estrada. 150 peças em aço cromo vanádio.',
    image:
...
export default function SeedDatabase() {
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      for (const cat of sampleCategories) {
        try {
          ...
          if (res.ok) {
            ...
            if (res.status === 401) {
              throw new Error(
                'Você precisa estar logado como administrador para executar o seed. Faça login primeiro em /admin.'
              );
            }
          }
        } catch (err) {
      ...
      addLog(
        `✨ Processo finalizado! Categorias: ${categoriesCreated}, Produtos: ${productsCreated}`
      );
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 mb-8">
          ...
          <p className="text-gray-300 mb-4">
            Esta ferramenta irá tentar popular o banco de dados com categorias e produtos de
            exemplo. Use apenas se o banco estiver vazio ou para restaurar dados perdidos.
          </p>
          <p className="text-sm text-gray-400 mb-6">
            Nota: É necessário permissão de administrador. Se falhar com erro 401, faça login no
            /admin primeiro.
          </p>
          ...
          {logs.length === 0 ? (
            ...
            <div className="space-y-1">
              {logs.map((log, i) => (
                <div
                  key={i}
                  className={`${log.includes('❌') ? 'text-red-400' : log.includes('✅') ? 'text-green-400' : 'text-gray-300'}`}
                >
                  {log}
                </div>
              ))}
